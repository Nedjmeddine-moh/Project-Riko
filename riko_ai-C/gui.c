/*
 * gui.c — Riko AI GTK4 GUI  (C rewrite)
 *
 * Build:
 *   gcc $(pkg-config --cflags --libs gtk4 json-glib-1.0) gui.c -o riko_gui
 *
 * Arch deps:
...
on_delete_key_row(GtkButton *btn, gpointer user_data)
{
    ...
    if (s->key_rows->len > 0) {
        KeyRowData *first = g_ptr_array_index(s->key_rows, 0);
        s->first_radio = first->radio;
        for (guint i = 1; i < s->key_rows->len; i++) {
            KeyRowData *r = g_ptr_array_index(s->key_rows, i);
        ...
        if (!any) {
            KeyRowData *first2 = g_ptr_array_index(s->key_rows, 0);
...
on_settings_save(GtkButton *btn, gpointer user_data)
{
    ...
    gint active_idx = 0;
    for (guint i = 0; i < s->key_rows->len; i++) {
        KeyRowData *row = g_ptr_array_index(s->key_rows, i);
