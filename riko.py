...
class Riko:
    def __init__(self, system_prompt=None):
        self.client = Groq()
        # Always store memory next to riko.py, regardless of working directory
        self.memory_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "riko_memory.json")
        self.memory = self.load_memory()

        # Use custom system prompt if provided, otherwise fall back to default
        prompt = system_prompt.strip() if system_prompt and system_prompt.strip() else self.get_personality_prompt()
